...
class FFCut(Scene):
    def construct(self):
        ...
        path1 = [[0, 3], [3, 1]]
        indices_1 = [network.edge_endpts.index(u) for u in path1]
        self.play(*[Indicate(network.Edges[k]) for k in indices_1], run_time = 1.5)

        self.wait(2)

        path2 = [[0, 3], [2, 3]]
        indices_2 = [network.edge_endpts.index(u) for u in path2]
        self.play(*[Indicate(network.Edges[k]) for k in indices_2], run_time = 1.5)
