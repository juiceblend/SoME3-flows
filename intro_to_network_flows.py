...
class Terminology(Scene):
    def construct(self):
        ...
        vertex = Tex("Vertex")
        vertex.shift(3*UP + 4*LEFT)
        edge = Tex("Edge")
        edge.shift(2.5*UP)
        flow = Tex("Maximum flow?")
        flow.shift(3.5*DOWN)
        highlighted_edge = [[1,4]]

        self.play(Write(VGroup(Nodes, Edges)), run_time = 1)
        self.wait(10)

        
        self.play(AnimationGroup(*[Indicate(Nodes[1]), FadeIn(vertex)], lag_ratio=0))
        self.wait(1.2)
        animations_1 = network.AugmentPath(highlighted_edge)
        self.play(AnimationGroup(*[animations_1[0], FadeIn(edge)], lag_ratio=0))
        self.wait(4.5)
        self.play(Wiggle(Edges[3]))
        self.wait(1)
        self.play(AnimationGroup(*[Indicate(Nodes[i]) for i in range(0,7)], lag_ratio=0.1))
        self.play(AnimationGroup(*[Indicate(Edges[i]) for i in range(0,12)], lag_ratio=0.05, run_time=1))

        self.wait(1)
        self.play(Indicate(Nodes[0]))
        self.wait(0.5)
        self.play(Indicate(Nodes[6]))
        self.wait(4)
        self.play(AnimationGroup(*[Wiggle(Edges[i]) for i in range(0,12)], lag_ratio=0.05))
        self.wait(10)
